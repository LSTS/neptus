...
public class MissionRenderer extends JPanel implements ActionListener, ChangeListener, VehicleStateListener {
	...
	private PlanStateGenerator stateGenerator = null;
	...
	public MissionRenderer(PlanType plan, MapGroup map, int shownRenderers) {
		...
		if (plan != null) {
			...
			stateGenerator = new PlanStateGenerator(initState, plan, renderers);		
			stateGenerator.setActionListener(this);
			stateGenerator.setDelay(delay);
			stateGenerator.pause();
	...
	public void actionPerformed(ActionEvent action) {
		...
		if ("follow".equals(action.getActionCommand())) {
		    if (followMode.isSelected()) {
		    	for (int i = 0; i < renderers.length; i++)  {
		    		if (stateGenerator != null && plan != null)
		    			renderers[i].followVehicle(plan.getVehicleType().getId());
		    		else {
		...
		if ("findVehicle".equals(action.getActionCommand())) {
	    	for (int i = 0; i < renderers.length; i++) {
	    		if (stateGenerator != null)
	    			renderers[i].focusLocation(stateGenerator.curState.getPosition());
	    		else {
	    			...
	    			/*
	    			if (getMainVehicle() != null) {
	    				VehicleState vState = (VehicleState) shownVehicles.get(getMainVehicle());
	    				if (vState != null)
	    					renderers[i].focusLocation(vState.getPosition());
	    			}
	    			*/
	    		}	    		
	    	}
	    	if (stateGenerator == null)
	    		vehicleIterator = (vehicleIterator+1)%shownVehicles.size();
		}
