 ...
 * Created by pdias
 * 12/10/2011
 * $Id:: TileMapQuest.java 9615 2012-12-30 23:08:28Z pdias                      $:
 */
 ...
 *
 */
@MapTileProvider(name = "MapQuest")
public class TileMapQuest extends TileHttpFetcher {
...
//  Note: MapQuest has 4 subdomains set up, otile1 to otile4, all pointing to the same CDN. Just like with OSM's a.tile to c.tile subdomains, these subdomains are provided to get around browser limitations on the number of simultaneous HTTP connections to each "host". Browser-based applications can thus request multiple tiles from multiple subdomains faster than from one subdomain.
    ...
    protected static String tileClassId = TileMapQuest.class.getSimpleName();

    private static Map<String, TileMapQuest> tilesMap = Collections.synchronizedMap(new HashMap<String, TileMapQuest>());

    private static final int MAX_LEVEL_OF_DETAIL = 18;

    public TileMapQuest(Integer levelOfDetail, Integer tileX, Integer tileY, BufferedImage image) throws Exception {
     ...
     * @throws Exception
     */
    public TileMapQuest(String id) throws Exception {
        super(id);
    }
    ...
    protected String createTileRequestURL() {
        ...
        // http://otile1.mqcdn.com/tiles/1.0.0/osm/14/7801/6129.png
        int server = (int) (4 * rnd.nextFloat());
        String urlGet = "http://otile" + server + "." + "mqcdn.com/tiles/1.0.0/osm/" + levelOfDetail + "/"
                + tileX + "/" + tileY + ".png";
        return urlGet;
    }
