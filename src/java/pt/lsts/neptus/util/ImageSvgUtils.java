...
public class ImageSvgUtils {
     ...
     */
    public static SVGDiagram getSvgImage(String path) {
        try {
            File svg = new File(Objects.requireNonNull(FileUtil.getResourceAsFileKeepName(path)));
            return getSvgImage(svg);
        }
        catch (Exception e) {
    ...
    public static SVGDiagram getSvgImage(File file) {
        try {
            SVGUniverse universe = new SVGUniverse();
            return universe.getDiagram(universe.loadSVG(file.toURL()));
        }
        catch (Exception e) {
            NeptusLog.pub().error(e);
            System.out.println("error loading svg!");
    ...
    public static BufferedImage getSvgImageAsBufferedImage(String path, BiFunction<Graphics2D, SVGDiagram, Void> graphicModifier) {
        ...
        if (diagram == null)
            return null;
        BufferedImage bufferedImage = ImageUtils.createCompatibleImage((int) Math.floor(diagram.getWidth()), (int) Math.floor(diagram.getHeight()),
     ...
     */
    public static void paintSvgImageToBufferedImage(Graphics2D graphics2D, SVGDiagram diagram) {
     ...
     */
    public static void fillSvgElementAttribute(SVGElement element, String attribute, String value, boolean inlineOrStyleType) {
        int attribType = inlineOrStyleType ? 0 : 1;
        try {
            if (element.hasAttribute(attribute, attribType))
                element.setAttribute(attribute, attribType, value);
            else
                element.addAttribute(attribute, attribType, value);
        }
        catch (SVGException e) {
            NeptusLog.pub().error(e);
        }
