 ...
 * <P>
 * <B>Implementation note:</B> The default number of items to display at a time is 15, and the default scrolling
 * interval is 125 milliseconds.
 * <P>
 * 
 * @author Darryl from http://tips4java.wordpress.com/2009/02/01/menu-scroller/
 */
public class MenuScroller {
    ...
    public static MenuScroller setScrollerFor(JMenu menu, Component comp, int interval, int topFixedCount,
            int bottomFixedCount) {
        Rectangle screenB = GuiUtils.getScreenBounds(comp.getX(), comp.getY());
        int s = (int) (screenB.getHeight() / 22) - 1;
        return new MenuScroller(menu, s, interval, topFixedCount, bottomFixedCount);
    }
    ...
    public static MenuScroller setScrollerFor(JPopupMenu menu, Component comp, int interval, int topFixedCount,
            int bottomFixedCount) {
        Rectangle screenB = GuiUtils.getScreenBounds(comp.getX(), comp.getY());
        int s = (int) (screenB.getHeight() / 22) - 1;
        return new MenuScroller(menu, s, interval, topFixedCount, bottomFixedCount);
    ...
    /**
     * Scrolls the specified item into view each time the menu is opened. Call this method with <code>null</code> to
     * restore the default behavior, which is to show the menu as it last appeared.
     * 
    ...
    private void refreshMenu() {
        if (menuItems != null && menuItems.length > 0) {
            ...
            firstIndex = Math.min(menuItems.length - bottomFixedCount - scrollCount + 2, firstIndex); // (pdias) +2 to
                                                                                                      // count the up
                                                                                                      // and down items

            upItem.setEnabled(firstIndex > topFixedCount);
            downItem.setEnabled(firstIndex + scrollCount < menuItems.length - bottomFixedCount + 2); // (pdias) +2 to
                                                                                                     // count the up and
                                                                                                     // down items
    ...
    public static void main(String[] args) {
        Rectangle screenB = GuiUtils.getScreenBounds(0, 0);
        ...
        file.setText("File");
        for (int i = 0; i < 100; i++) {
            file.add(new JMenuItem("ssssssssss " + (i + 1)));
        }
        menu.add(file);
        setScrollerFor(file, 20, 125, 1, 0);
        ...
        int s = (int) (screenB.getHeight() / 21) - 1;
        file = new JMenu();
        file.getPopupMenu().setLightWeightPopupEnabled(false);
        file.setText("File h1 100:" + s);
        for (int i = 0; i < 100; i++) {
            file.add(new JMenuItem("ssssssssss " + (i + 1)));
        }
        menu.add(file);
        setScrollerFor(file, s, 125, 1, 0);

        s = (int) (screenB.getHeight() / 21);
        file = new JMenu();
        file.getPopupMenu().setLightWeightPopupEnabled(false);
        file.setText("File h1 100:" + s);
        for (int i = 0; i < 100; i++) {
        ...
        menu.add(file);
        setScrollerFor(file, s, 125, 0, 0);

        file = new JMenu();
        file.getPopupMenu().setLightWeightPopupEnabled(false);
        file.setText("File h1 38:" + s);
        for (int i = 0; i < 38; i++) {
            file.add(new JMenuItem("ssssssssss " + (i + 1)));
        }
        menu.add(file);
