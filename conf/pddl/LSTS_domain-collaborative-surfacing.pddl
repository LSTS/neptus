...
(:predicates (at ?v - vehicle ?l - location) ;position of the vehicle
             ...
             (collab_at ?i - interest ?l - location) ; a location where the "collaborative" vehicle should be during the "collaborative" survey or sample action
             ...
             (collab_task_desc ?t - task ?i - interest ?p - payload) ; a description of the collaborative task that sample (or survey) the point of interest by the payload
...
(:functions (distance ?l1 ?l2 - location)
            ...
            (surface-credits ?v - vehicle) ; how many sample actions the vehicle can perform before surfacing
            (surface-credits-max ?v - vehicle) ; how many sample actions at most the vehicle can perform before surfacing
...
(:durative-action move-to-collab
:parameters (?v - vehicle ?l1 ?l2 - location ?i - interest)
:duration (= ?duration (/ (distance ?l1 ?l2)(speed ?v)))
:condition (and (at start (at ?v ?l1))
                (at start (can-move ?v))
                (at end (free ?l2))
                (over all (collab_at ?i ?l2))
                (over all (not (= ?l1 ?l2)))
                ;(over all (<= (from-base ?v)(max-to-base ?v)))
                ;(at start (< (+ (from-base ?v)(/ (distance ?l1 ?l2)(speed ?v))) (max-to-base ?v)))
                ;(at start (>= (battery-level ?v)(* (battery-consumption-move ?v)(distance ?l1 ?l2))))
...
:effect (and (at start (not (at ?v ?l1)))
             ...
             ;(at start (increase (from-base ?v)(/ (distance ?l1 ?l2)(speed ?v))))
             ;(at start (decrease (battery-level ?v)(* (battery-consumption-move ?v)(distance ?l1 ?l2))))
             ;(at end (assign (surface-credits ?v)(surface-credits-max ?v)))
        )
)
...
:effect (and (at start (not (at ?v ?l1)))
             ...
             (at end (not (free ?l2)))
             (at end (not (can-move ?v)))
             (at end (assign (surface-credits ?v) 0))
             ;(at start (increase (from-base ?v)(/ (distance ?l1 ?l2)(speed ?v))))
             ;(at start (decrease (battery-level ?v)(* (battery-consumption-move ?v)(distance ?l1 ?l2))))
        )
)
...
:effect (and (at start (not (at ?v ?l1)))
             ...
             (at start (increase (base-returns) 200))
             ;(at start (increase (from-base ?v)(/ (distance ?l1 ?l2)(speed ?v))))
...
:condition (and (at start (<= (surface-credits ?v) 0)))
:effect (and (at end (assign (surface-credits ?v)(surface-credits-max ?v))))
)
...
:effect (and (at end (sampled ?t ?v))
             (at end (can-move ?v))
             (at end (decrease (surface-credits ?v) 1))
            ; (at start (increase (from-base ?v) 60))
            ; (at start (decrease (battery-level ?v)(* (battery-consumption-payload ?p) 10)))
        )
)

(:durative-action collab-sample
:parameters (?v - vehicle ?l - location ?t -task ?o -oi ?p - payload ?v2 - vehicle ?l2 - location)
:duration (= ?duration 60)
:condition (and (over all (at_oi ?o ?l))
                (over all (collab_task_desc ?t ?o ?p))
                (over all (at ?v ?l))
                (over all (collab_at ?o ?l2))
                (over all (at ?v2 ?l2))
                (over all (having ?p ?v))
                (at start (> (surface-credits ?v) 0))
               ; (over all (<= (from-base ?v)(max-to-base ?v)))
               ; (over all (<= (from-base ?v2)(max-to-base ?v2)))
                ;(at start (< (+ (from-base ?v) 60) (max-to-base ?v)))
              ;  (at start (>= (battery-level ?v)(* (battery-consumption-payload ?p) 10)))
           )
:effect (and (at end (sampled ?t ?v))
             ...
             (at end (decrease (surface-credits ?v) 1))
            ; (at start (increase (from-base ?v) 60))
            ; (at start (increase (from-base ?v2) 60))
            ; (at start (decrease (battery-level ?v)(* (battery-consumption-payload ?p) 10)))
             (at end (assign (surface-credits ?v2)(surface-credits-max ?v2)))
        )
)
...
(:durative-action collab-survey-one-payload
:parameters (?v - vehicle ?l1 ?l2 - location ?t -task ?a -area ?p - payload ?v2 - vehicle ?l3 - location)
:duration (= ?duration (/ (surveillance_distance ?a)(speed ?v)))
:condition (and (over all (entry ?a ?l1))
                (over all (exit ?a ?l2))
                (over all (collab_at ?a ?l3))
                (over all (at ?v2 ?l3))
                (over all (having ?p ?v))
                (over all (collab_task_desc ?t ?a ?p))
                (at start (at ?v ?l1))
                (at end (free ?l2))
                (at start (available ?a))
                (at start (> (surface-credits ?v) 0))
...
:effect (and (at end (sampled ?t ?v))
             ...
             (at end (assign (surface-credits ?v) 0))
             ;(at start (increase (from-base ?v)(/ (surveillance_distance ?a)(speed ?v))))
            ; (at start (increase (from-base ?v2)(/ (surveillance_distance ?a)(speed ?v))))
             ;(at start (decrease (battery-level ?v)(+ (* (battery-consumption-move ?v)(surveillance_distance ?a))(* (battery-consumption-payload ?p) (/ (surveillance_distance ?a)(speed ?v))))))
             (at end (assign (surface-credits ?v2)(surface-credits-max ?v2)))
         )
)
...
(:durative-action collab-survey-two-payload
:parameters (?v - vehicle ?l1 ?l2 - location ?t ?t2 -task ?a -area ?p ?p2 - payload ?v2 - vehicle ?l3 - location)
:duration (= ?duration (/ (surveillance_distance ?a)(speed ?v)))
:condition (and (over all (entry ?a ?l1))
                (over all (exit ?a ?l2))
                (over all (collab_at ?a ?l3))
                ...
                (over all (collab_task_desc ?t ?a ?p))
                (over all (having ?p2 ?v))
                (over all (collab_task_desc ?t2 ?a ?p2))
                (over all (not (= ?t ?t2)))
                (at start (at ?v ?l1))
                (at end (free ?l2))
                (at start (available ?a))
                (at start (> (surface-credits ?v) 0))
               ; (over all (<= (from-base ?v)(max-to-base ?v)))
               ; (over all (<= (from-base ?v2)(max-to-base ?v)))
                ;(at start (< (+ (from-base ?v)(/ (surveillance_distance ?a)(speed ?v))) (max-to-base ?v)))
                ;(at start (>= (battery-level ?v)(+ (* (battery-consumption-move ?v)(surveillance_distance ?a))(* (+ (battery-consumption-payload ?p)(battery-consumption-payload ?p2)) (/ (surveillance_distance ?a)(speed ?v))))))
          )
:effect (and (at end (sampled ?t ?v))
...
(:durative-action collab-survey-three-payload
:parameters (?v - vehicle ?l1 ?l2 - location ?t ?t2 ?t3 -task ?a -area ?p ?p2 ?p3 - payload ?v2 - vehicle ?l3 - location)
:duration (= ?duration (/ (surveillance_distance ?a)(speed ?v)))
:condition (and (over all (entry ?a ?l1))
                (over all (exit ?a ?l2))
                (over all (collab_at ?a ?l3))
                ...
                (over all (collab_task_desc ?t2 ?a ?p2))
                (over all (having ?p3 ?v))
                (over all (collab_task_desc ?t3 ?a ?p3))
                (over all (not (= ?t ?t2)))
                (over all (not (= ?t ?t3)))
                (over all (not (= ?t3 ?t2)))
                (at start (at ?v ?l1))
