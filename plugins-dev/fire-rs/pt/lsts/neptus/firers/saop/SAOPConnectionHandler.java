...
public class SAOPConnectionHandler extends ConsoleLayer {
            ...
            public void onMessage(MessageInfo info, IMCMessage msg) {
                ...
                // }
                    ...
                    try {
                        ...
                        for (ContourLine contour : polygons) {
    ...
    private ContourLine processContourLine(JsonObject contourLineObj) {
        ...
        poly.addVertex(poly.getVertices().get(0).getLatitudeDegs(), poly.getVertices().get(0).getLongitudeDegs()); // adding
                                                                                                                   // first
                                                                                                                   // point
                                                                                                                   // to
                                                                                                                   // close
...
//            imgbuff = op.filter(imgbuff, null);
            ...
            g.drawImage(imgbuff, 0, 0, new Double(raster.xSize * raster.getCellWidth()).intValue(),
                    new Double(raster.ySize * raster.getCellWidth()).intValue(), renderer);
        }
    ...
    private class FireRaster {
        ...
        public int[] getScaleFactor(int maxWidth, int maxHeight) {
            int[] res = new int[2];
            double imgRatio = (double) raster.xSize * raster.getCellWidth() / (double) raster.ySize
                    * raster.getCellWidth();
