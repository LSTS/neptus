...
public class Util {
    ...
    static Camera parseLineCamera(String line) {
        ...
        if (splits.length == 3) {
            ...
            if (getHostFromURI(splits[2].trim().trim()) == null) return null;
            ...
            if (splits[0].trim().isEmpty()) return null;
            if (splits[1].trim().isEmpty()) return null;
            String host = getHostFromURI(splits[1].trim().trim());
    ...
    public static void addCamToFile(Camera camToAdd, String fileName) {
        String iniRsrcPath = FileUtil.getResourceAsFileKeepName(VideoReader.BASE_FOLDER_FOR_URL_INI);
    ...
    public static BufferedImage resizeBufferedImage(BufferedImage img, Dimension size) {
    ...
    static synchronized void createIpUrlFile() {
        String iniRsrcPath = FileUtil.getResourceAsFileKeepName(VideoReader.BASE_FOLDER_FOR_URL_INI);
